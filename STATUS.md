...
> **Last updated:** 2026-06-06 (M6-content addon scaffolding landed; M5-D paused mid-rerun pending VM bring-up)
...
- **M6-content done** (2026-06-06, this session): addon packs (DLC) — Hermes 3 + Mistral + offline Simple Wikipedia. Reuses M2's `packs/recon/` debhelper-compat-13 metapackage skeleton but extends it for two new content kinds that aren't apt artifacts: Ollama model tags + Kiwix ZIM blobs.
  ...
  - **Follow-ons for a future M6 commit**: package `minti-pack-fetch` itself as a .deb (then re-add `Depends: minti-pack-fetch`); deploy `minti-pack-fetch` from `install.sh` (currently it's in `pack-manager/dist/` only); pin Kiwix ZIM SHA-256 (manifest currently has it empty; the helper accepts missing checksums but skips verify); add `minti-pack-hermes3-70b` + `minti-pack-mistral-nemo` + `minti-pack-wiki-en-top` + `minti-pack-wiki-en` follow-on tiers; consider building inside an `lxc-launch debian:13` to dodge vboxsf entirely.
